...
Уточнение по п. 9: captures нужны как локальный исследовательский материал для
проверки fingerprint; включать их в репозиторий или распространять не требуется.
Новый пятираундовый benchmark и результаты приведены в `MVP0_ACCEPTANCE.md`;
прежние значения получены при разных TLS mode и не используются для вывода об
overhead. Статус № 9 уточнён: локальный порог overhead пройден; остаётся
проверка fingerprint на локальных captures устройств. Публикация или
распространение capture-файлов не требуется.

Сверка среды 2026-09-26: доступная команда `go` возвращает `go1.26.6
windows/amd64`. Записанный ниже полный quality gate на Go 1.27.1 относится к
предыдущей проверке; в текущей сессии `go test ./...`, `go vet ./...` и
`go build ./...` повторно прошли с Go 1.26.6.

Уточнение по п. 19: локальный lifecycle-provider сообщает метаданные и срок
действия PEM-сертификатов, атомарно заменяет отдельные secret-файлы и позволяет
явно ротировать combined PEM CA через `POST /api/v1/admin/ca/rotate` или кнопку
панели. Новый CA активируется после атомарной записи; клиентам нужно установить
новый сертификат. При раздельных путях cert/key ротация отказывает без изменений.
Проверки: `TestFileProviderReportsCertificateExpiryWithoutReturningSecret`,
`TestFileProviderRotatesExistingSecretAtomicallyAndPreservesMode`,
`TestRotateCertificateAuthorityUsesLifecycleProviderAtomically`,
`TestRotateCAUsesConfiguredLifecycleProviderAndUpdatesActiveCA`,
`TestCARotateRequiresAdminAndReportsClientTrustRequirement`.
...
| 9 | Частично реализовано; требуется проверка на реальных устройствах | Бинарный opt-in тест `TestProxyBinaryLoadMatrix` на локальном Windows/amd64 стенде (Go 1.26.6), HTTP и SOCKS5, 5 парных раундов, 1600 запросов на вариант: медианный overhead прошёл порог `<10%` для throughput и p95; ошибок и потерь нет, по 4800 наблюдений записано в каждом enabled-прогоне. Результат и прежняя неудачная серия приведены в `MVP0_ACCEPTANCE.md`; порог подтверждён этим запуском, но не гарантируется для иных машин и профилей нагрузки. Для проверки точности fingerprint остаются локальные реальные Safari/iOS и Android/OkHttp captures; synthetic fixtures их не заменяют. |
...
| 19 | Реализовано для локального file-backed provider | `Provider` сохраняет совместимость; дополнительный `LifecycleProvider` предоставляет метаданные и атомарную замену существующего файла. `FileProvider.Metadata` сообщает даты/размер, а для PEM-сертификата — `NotAfter`; TLS CA и сертификат панели показывают `VALID`/`EXPIRING`/`EXPIRED`/`NOT_YET_VALID`, токены и spool key имеют собственные сроки/статусы. Combined PEM CA можно явно ротировать через `POST /api/v1/admin/ca/rotate` или кнопку панели; замена атомарна, активное состояние обновляется только после записи, при раздельных путях endpoint отказывает. Клиентам нужно установить новый публичный CA. File-backed bootstrap token, spool key, CA, HTTPS keypair и upstream/proxy credentials используют общий `Provider`; secret values исключаются из backup и логов. Тесты включают `TestFileProviderReportsCertificateExpiryWithoutReturningSecret`, `TestFileProviderRotatesExistingSecretAtomicallyAndPreservesMode`, `TestRotateCertificateAuthorityUsesLifecycleProviderAtomically`, `TestRotateCAUsesConfiguredLifecycleProviderAndUpdatesActiveCA`, `TestCARotateRequiresAdminAndReportsClientTrustRequirement`, `TestWebPanelTLSBundleUsesOneSnapshotPerHandshake`, `TestSpoolKeyRotationRequiresDrainedRecords`. Внешний secret backend и фоновая замена CA нормативным п. 19 не требуются. |
| 20 | Реализовано частично; ручная визуальная проверка не выполнена | В интерфейсе унифицированы русские подписи лаборатории TLS, профилей, регистратора и токенов; динамические фильтры сессий объявляют состояние через `aria-pressed`; для главных вкладок работают стрелки, Home и End; таблицы имеют доступные названия; в `app.css` учтена системная настройка `prefers-reduced-motion`. Статусные и фокусные цвета проверены расчётом контраста: пары в основных экранах профилей/регистратора — не ниже 4,5:1; фокусная рамка составляет 7,27:1 на белом фоне и 7,21:1 на тёмной шапке. В README/docs/.github не найдено упоминаний Docker/контейнеров; проверены 11 Markdown-файлов, битых локальных ссылок нет. Проверки: расчёт контрастности выбранных цветовых пар, `node --check` для статических JS, `git diff --check`, `go test ./internal/ja3proxy/webpanel/...`. Остаётся проверить визуальную согласованность экранов и клавиатурные сценарии в браузере. |

Актуализация подпункта 19 после таблицы (она заменяет прежнюю пометку об
отсутствии expiry ключа spool): `--capture-spool-key-max-age` задаёт
необязательный срок ключа spool; `0` (по умолчанию) отключает контроль.
Активация и expiry отражаются в recorder stats и панели (`VALID`, `EXPIRING`,
`EXPIRED`, `UNKNOWN`, `UNCONFIGURED`). Expiry информативен и не блокирует
запись/replay; ротация по-прежнему разрешена только при пустом spool. Формат
`key.id` расширен до метаданных с обратной совместимостью: legacy marker
использует свой mtime как приближение даты активации, а для legacy spool без
достоверной даты сохраняется `UNKNOWN`. Проверки: `TestSpoolKeyExpiryIsAdvisoryAndPersistsAcrossRestart`,
`TestLegacyPendingSpoolKeyExpiryRemainsUnknown`, `TestRecorderStatsExposeSpoolKeyExpiry`.
Входящие proxy credential-файлы также можно применить без перезапуска через
`POST /api/v1/admin/proxy-auth/reload` или кнопку настроек. Проверка обоих
значений выполняется до атомарной замены; ошибка сохраняет активную пару,
ссылка на file-backed конфигурацию остаётся, а открытые соединения не затрагиваются.
Тесты: `TestReloadProxyCredentialsAppliesFilesAtomically`,
`TestProxyAuthReloadAPIRequiresAdminAndAudit`. File-backed credentials глобального
upstream применяются без перезапуска через `POST
/api/v1/admin/upstream-credentials/reload` или кнопку панели. Новый dialer
создаётся до атомарной замены текущего, а успешная смена сбрасывает кэш
route-specific dialer для перечитывания их ссылок; активные соединения не
прерываются. Кнопка доступна при настроенных `file:` credentials именно у
глобального upstream. Тесты: `TestDynamicUpstreamDialerDetectsFileCredentials`,
`TestReloadUpstreamCredentialsRebuildsDialersAndPreservesOnFailure`,
`TestUpstreamCredentialReloadAPIRequiresAdminAndAudit`.
...
JSONL пока не шифруется. Размещайте экспорт в контролируемом каталоге; raw содержит session identifiers/tickets. Для отказоустойчивости SQLite доступен явно включаемый AES-256-GCM spool с bounded quota. File-backed secret provider, метаданные срока действия сертификатов, явная ротация combined PEM CA и runtime reload credentials реализованы; подробности и ограничения приведены в разделе пункта 19 выше.
...
| SEC-AUTH-003: отзыв registry-токена без удаления записи | `AuthToken.Revoked`, SQLite token registry (JSON-файл — bootstrap) | `TestTokenAuthRejectsRevokedRegistryToken` |
...
Сервис также передаёт собранные UDP DNS datagrams и TCP DNS frames в DNS sensor;
при совместном включении recorder и packet capture TLS-наблюдения связываются с
подходящими DNS-ответами, как описано ниже. Захват интерфейсов вне Windows/Linux
не реализован; forwarding не меняется.
...
1. Для полной приёмки MVP-0: реальные captures Safari/iOS и Android/OkHttp для
   проверки device fingerprint и дополнительные handshake-сценарии. Полная
   performance-матрица recorder-on/off (p95/throughput) выполнена на локальном
   стенде; результаты и ограничения обобщения описаны в `MVP0_ACCEPTANCE.md`.
