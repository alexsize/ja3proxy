...
| 19 | Частично реализовано; Go quality gate пройден | Добавлен общий `secrets.Provider` и локальный `FileProvider` с проверкой regular-file и лимитом 1 МиБ. Интегрирован в загрузку bootstrap token file, ключа encrypted spool, CA key pair и HTTPS panel key pair; HTTPS-сертификат перечитывается на каждый новый TLS handshake. Срок действия MITM CA и HTTPS-сертификата панели вычисляется со статусами `VALID`/`EXPIRING`/`EXPIRED`/`NOT_YET_VALID` и отображается в `/api/state` и настройках панели; предупреждение `EXPIRING` выдаётся за 30 дней. Upstream username/password разрешаются через Provider из `file:` ссылок в URL userinfo при создании dialer, поддерживаются динамическим и route-specific upstream. Входящая proxy-аутентификация принимает пары `--proxy-username-file`/`--proxy-password-file` через Provider. Backup redaction удаляет URL userinfo целиком, включая ссылки и credentials. Целевые и полные unit-тесты, build, vet и race-прогон проверены. Для HTTPS панели общий PEM-файл с сертификатом и ключом считывается один раз на handshake, что позволяет атомарно опубликовать согласованную пару заменой файла; `TestWebPanelTLSBundleUsesOneSnapshotPerHandshake` проверяет ротацию между handshake. При отдельных файлах атомарность пары не гарантирована. Spool привязывается к отпечатку ключа в `key.id`: при ожидающих записях смена ключа отклоняется без изменения данных, после drain ключ можно заменить при перезапуске; для legacy spool без маркера ключ проверяется по записи. Проверки: `TestSpoolKeyRotationRequiresDrainedRecords`, `TestLegacySpoolKeyMustDecryptPendingRecordBeforeBinding`. Для MITM CA одинаковые `--ca-cert`/`--ca-key` теперь создают и загружают один PEM с атомарной публикацией, при ошибке записи прежний CA остаётся активным; это проверяют `TestGenerateCombinedCABundleAndRotate`, `TestFailedCombinedCAGenerationKeepsLoadedCA`, `TestEnsureCAWithCombinedBundle`. MITM CA теперь можно применить без перезапуска через аудируемый `POST /api/v1/admin/ca/reload`: non-loopback требует admin, loopback остаётся без обязательного токена. Пара сертификат/ключ подменяется под RW-lock; незавершённая выдача использует только согласованную старую или новую пару, а ошибка загрузки сохраняет прежний CA. Проверки: `TestCAReloadKeepsIssuerAndSignerTogether`, `TestCAReloadRequiresAdminRemotelyAndAllowsLoopback`, `TestCAReloadFailsClosedWithoutAuditAndPreservesErrorDetails`, `TestReloadCAKeepsOldOnFailureAndAppliesNewBundle`. Клиенты должны обновить доверие к новому CA. В настройках панели добавлена кнопка применения подготовленного CA с подтверждением, результатом и обновлением статуса; `TestHandlerServesPanel` проверяет наличие UI и вызова API. Ключ spool применяется без перезапуска через `POST /api/v1/admin/spool/reload-key` и кнопку в настройках: смена разрешена только при пустом spool, сериализована с append/replay, сохраняет старый ключ при ошибке публикации и требует аудит/admin вне loopback. Проверки: `TestReloadSpoolKeyPreservesPendingRecordsAndSurvivesRestart`, `TestSpoolRotationPublicationFailureKeepsOldKey`, `TestSpoolRotationSerializesWithAppend`, `TestSpoolKeyReloadAPIHonorsAdminAndAudit`. Не реализованы автоматическое продление сертификатов, expiry для spool key и других секретов и внешний secret backend. |
