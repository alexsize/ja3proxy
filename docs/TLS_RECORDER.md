...
Необязательный `--capture-spool-key-max-age 8760h` задаёт срок действия ключа
(пример — один год); значение `0` отключает контроль. Статус и даты активации/
истечения публикуются в статистике recorder как `spool_key_status`,
`spool_key_activated_at`, `spool_key_expires_at` (`VALID`, `EXPIRING`,
`EXPIRED`, `UNKNOWN`, `UNCONFIGURED`). Это предупреждение, а не блокировка:
истёкший ключ продолжает обслуживать запись и replay, чтобы не терять данные.
Ротировать его можно по обычной процедуре только после доставки всех `.evt`.
Для legacy spool с ожидающими записями без достоверной даты активации статус
будет `UNKNOWN`; дата не подменяется временем миграции.
...
обновляется автоматически. Маркер содержит SHA-256 отпечаток и, если известна,
дату активации — секрет в нём не хранится. Для старого spool без `key.id` ключ
проверяется по ожидающей записи до создания маркера; если проверка невозможна, записи не
...
в audit и не публикуются API. JSON-файл служит для первоначальной загрузки;
после импорта источником истины становится SQLite, где хранится SHA-256 digest
токенов.
...
а загрузка читает его как одну пару. Чтобы создать и сразу активировать новую
пару, используйте кнопку «Создать и активировать новый CA» или
`POST /api/v1/admin/ca/rotate`. Эта операция атомарно заменяет combined PEM;
при раздельных путях cert/key она отказывает без изменений. Новый публичный
сертификат можно скачать в панели или через `/api/v1/ca/certificate`.
Для ручной смены подготовьте новый согласованный PEM отдельно и атомарно
замените файл по тому же пути. Затем можно перезапустить приложение или применить
его к работающему прокси через
...
Фоновое продление сертификатов не выполняется. Для MITM CA доступна явная
ротация, ключ spool можно заменить после опустошения очереди, а file-backed
proxy/upstream credentials перечитываются соответствующими командами reload.
Локальный `FileProvider` сообщает метаданные файла и срок действия PEM-сертификата;
неизвестный срок для обычных текстовых credentials не вычисляется.
...
перевода строки) для глобального upstream применяется кнопкой «Применить
upstream credentials» в настройках или `POST
/api/v1/admin/upstream-credentials/reload`. Новые соединения используют новую
пару, существующие не прерываются; при ошибке старый dialer остаётся активным.
При успешной операции кешированные route-upstream dialer сбрасываются, чтобы
их файловые ссылки перечитались при следующем использовании. Кнопка доступна,
если глобальный upstream содержит хотя бы одну `file:` ссылку.
...
завершающий перевод строки станет частью логина или пароля. Для применения
изменённой пары без перезапуска используйте кнопку «Применить credential-файлы»
в настройках или `POST /api/v1/admin/proxy-auth/reload` без тела запроса.
Применение атомарное: если файл не читается или пара невалидна, прежние
учётные данные остаются активными. Новые данные действуют для новых HTTP- и
SOCKS5-аутентификаций; уже установленные соединения не прерываются. Повторное
применение доступно, если настроен хотя бы один `*-file` параметр.
...
- SQLite token registry поддерживает выдачу, просмотр, изменение роли/scopes,
  отзыв и runtime-ротацию токенов; секрет показывается только при выдаче.
  Отдельная модель пользовательских password/session-учётных записей не входит
  в текущий контур. Append-only audit-журнал действий панели уже реализован.
  Двухфазный route manager уже
